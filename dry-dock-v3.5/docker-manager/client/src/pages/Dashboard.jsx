...
export default function Dashboard({ onSwitchEnvironment }) {
  ...
  async function handleDragEnd() {
    ...
    try {
      await reorderEnvironments(cards.map((c) => c.id));
    } catch (err) {
    ...
    <div>
      <div className="section-heading">
        ...
        <div className="flex-row">
          {canManage && cards.length > 1 && (
            <button
              className={`btn btn-sm ${reordering ? "btn-primary" : "btn-ghost"}`}
              onClick={() => setReordering((r) => !r)}
            >
              {reordering ? "Done arranging" : "Manage nodes"}
            </button>
          )}
          {canManage && (
      ...
      {reordering && (
        <div className="banner" style={{ marginBottom: 16 }}>
          Drag a card to reorder it. The new order is saved automatically.
        </div>
      )}
      ...
      {showLoading ? (
        ...
        <div className="env-card-grid">
          {cards.map((card, index) => (
            <div
              className={`env-card ${reordering ? "env-card-draggable" : ""}`}
              key={card.id}
