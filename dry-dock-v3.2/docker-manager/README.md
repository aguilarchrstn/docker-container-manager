...
environments, activity log, theme, saved presets — as JSON files under
`server/data/`, one file per collection. That's the whole database: no
service to run, nothing to configure.

Set **`DATABASE_URL`** (a standard `postgresql://user:pass@host:port/db`
connection string) and Dry Dock switches to PostgreSQL instead, with zero
other changes needed — every route (`users.js`, `teams.js`, `roles.js`,
etc.) only ever calls a handful of shared read/write functions
(`server/lib/store.js`), and those transparently pick the backend based on
whether `DATABASE_URL` is set. Internally it's one table
(`dry_dock_store`, a `key`/`value jsonb` pair per collection) rather than a
bespoke relational schema — this mirrors the exact shape the JSON files
already had, which is what makes switching backends safe without a real
migration step. The table is created automatically on first boot if it
doesn't exist.

If you'd rather set discrete fields instead of a single connection string,
use `PG_HOST` / `PG_PORT` / `PG_USER` / `PG_PASSWORD` / `PG_DATABASE`
instead of `DATABASE_URL` — either form works, `DATABASE_URL` takes
priority if both are set.

The easiest way to turn this on is the Compose Generator's **Database**
step — toggle "Use external PostgreSQL database", fill in a name/user/
password/port, and it adds a real `postgres` service plus the
`DATABASE_URL` wiring to the generated Compose file for you.

**What this does *not* cover:** Stacks' compose files themselves
(`server/data/stacks/*.yml`) stay on disk regardless — `docker compose`
needs a real file path to run against, so those were never candidates for
moving into a database in the first place.
