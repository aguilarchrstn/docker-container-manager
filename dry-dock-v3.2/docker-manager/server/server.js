...
import { isDatabaseConfigured, ensureSchema, assertDatabaseReachable } from "./lib/db.js";
...
app.listen(PORT, async () => {
  ...
  if (isDatabaseConfigured()) {
    const reachable = await assertDatabaseReachable();
    if (reachable) await ensureSchema();
    else logger.error("Falling back is not possible once DATABASE_URL is set — fix the connection and restart.");
  }
  await seedDefaults();
