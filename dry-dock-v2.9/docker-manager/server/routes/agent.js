...
// Host-level system stats (CPU/memory/swap/disk) for this agent's own
// node — or forwarded on if ?env points at a further chained environment.
agentRouter.get("/system-stats", async (req, res) => {
  try {
    const envId = req.query.env || "local";
    const target = await getEnvironment(envId);
    if (!target) return res.json({ available: false, reason: `Unknown environment: ${envId}` });

    if (target.type === "agent") {
      const base = (target.config?.baseUrl || "").replace(/\/$/, "");
      const remoteEnvId = target.config?.remoteEnvironmentId || "local";
      const upstream = await fetch(`${base}/api/agent/system-stats?env=${encodeURIComponent(remoteEnvId)}`, {
        headers: { "x-agent-token": target.config?.agentToken || "" },
      });
      const body = await upstream.json();
      return res.json(body);
    ...
    if (target.type === "standalone") {
      return res.json({
        available: false,
        reason: "Host-level system stats aren't available over a direct standalone connection.",
      });
    }
...
networksRouter.use(resolveEnvironment, agentProxyPassthrough);
