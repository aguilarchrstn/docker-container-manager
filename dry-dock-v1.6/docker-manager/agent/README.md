...
A tiny companion service for [Dry Dock](https://github.com/) — install this
...
## Richer metrics with cAdvisor

By default this agent reports basic per-container stats straight from the
Docker Engine (CPU/mem/net/disk — same numbers `docker stats` shows). If
you already run [cAdvisor](https://github.com/google/cadvisor) on this
node (or want to), point the agent at it instead:

```bash
CADVISOR_URL=http://cadvisor:8080   # or http://localhost:8081 if that's how you published it
```

Set it in `.env` (see `.env.example`) and restart. The manager's
Monitoring page will automatically pick it up — no change needed on the
manager side — and shows which source is active. There's a commented-out
`cadvisor` service in `docker-compose.yml` if you don't already have one
running.
...
| `CADVISOR_URL`         | no       | —                          | Point at a cAdvisor instance for richer per-container metrics |

## Stacks (Docker Compose)

This agent has the `docker compose` CLI built in (the Dockerfile installs
`docker-cli` + `docker-cli-compose`), so when the manager's Stacks page
targets a node connected through this agent, the actual `docker compose up
-d` / `down` / etc. commands run here, against this node's own Docker
Engine. Stack definitions are stored under `/app/data/stacks` inside the
container — make sure that's on a persisted volume (the provided
`docker-compose.yml` already mounts `drydock-agent-data:/app/data` for
this) or you'll lose them on a rebuild.

Nothing to configure — it works automatically once this agent is connected
as an environment in the manager.
...
- `GET/POST/PUT/DELETE /api/agent/stacks...` — list/create/update/start/
  stop/remove/logs
