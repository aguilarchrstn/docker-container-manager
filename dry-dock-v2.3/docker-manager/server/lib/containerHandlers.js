...
export function registerContainerRoutes(router, { viewGuards = [], manageGuards = [] } = {}) {
  ...
  // Live-follows a container's logs over Server-Sent Events instead of
  // pulling a static tail — used by the "Follow" toggle in the Logs modal.
  // Demuxes Docker's multiplexed stdout/stderr frame protocol incrementally
  // as chunks arrive, since a single TCP chunk won't necessarily line up
  // with frame boundaries.
  router.get("/:id/logs/stream", ...viewGuards, async (req, res) => {
    try {
      ...
      function processBuffer() {
        while (buffer.length >= 8) {
          ...
          if (frameType > 2) {
            // Not the multiplexed stdout/stderr protocol (e.g. a TTY-enabled
            // container streams raw bytes) — just forward it as-is.
            sendLine(buffer.toString("utf-8"));
            buffer = Buffer.alloc(0);
            break;
          }
          const size = buffer.readUInt32BE(4);
          if (buffer.length < 8 + size) break; // wait for the rest of this frame
          sendLine(buffer.slice(8, 8 + size).toString("utf-8"));
