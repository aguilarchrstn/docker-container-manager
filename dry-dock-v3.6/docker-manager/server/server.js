...
import { isDatabaseConfigured, ensureSchema, waitForDatabase } from "./lib/db.js";
import { attachUser } from "./middleware/auth.js";
import { activityLogger } from "./middleware/activityLogger.js";
import { seedDefaults } from "./lib/seed.js";
import { startHealthPoller } from "./lib/healthPoller.js";
...
app.listen(PORT, async () => {
  ...
  try {
    if (isDatabaseConfigured()) {
      const reachable = await waitForDatabase({ attempts: 10, delayMs: 2000 });
      if (!reachable) {
        logger.error(
          "Giving up on PostgreSQL — Dry Dock cannot start with DATABASE_URL/PG_HOST set but unreachable " +
            "(there's no fallback to JSON files once a database is configured, to avoid silently splitting " +
            "your data across two places). Fix the connection and restart the container."
        );
        process.exit(1);
      }
      await ensureSchema();
